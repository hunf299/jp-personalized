...
export async function countRemainingByType(totals) {
  const totalsData = totals || await countByType();

  const { data, error } = await supabase
      .from('memory_levels')
      .select('card_id, type, cards:card_id(type, deleted)')
      .eq('cards.deleted', false);
  if (error) throw error;

  const learnedSets = new Map();
  (data || []).forEach((row) => {
    const cardIdRaw = row?.card_id;
    if (cardIdRaw == null) return;
    const type = row?.type || row?.cards?.type;
    if (!type) return;
    const key = String(type);
    if (!learnedSets.has(key)) learnedSets.set(key, new Set());
    learnedSets.get(key).add(String(cardIdRaw));
  });

  const remaining = {};
  Object.entries(totalsData || {}).forEach(([type, total]) => {
    if (type === 'total') return;
    const normalizedType = String(type);
    const totalCount = Number.isFinite(Number(total)) ? Number(total) : 0;
    const learned = learnedSets.get(normalizedType)?.size || 0;
    remaining[normalizedType] = Math.max(0, totalCount - learned);
  });

  learnedSets.forEach((set, type) => {
    if (Object.prototype.hasOwnProperty.call(remaining, type)) return;
    const totalCount = Number.isFinite(Number(totalsData?.[type])) ? Number(totalsData[type]) : 0;
    remaining[type] = Math.max(0, totalCount - set.size);
  });

  remaining.total = Object.values(remaining).reduce((sum, value) => sum + (Number.isFinite(Number(value)) ? Number(value) : 0), 0);
  ...
  if (rows.length){
    ...
    if (cardIds.length){
      const { data: existingMemory, error: existingErr } = await supabase
          .from('memory_levels')
          .select('card_id, leech_count, is_leech')
          .in('card_id', cardIds);
