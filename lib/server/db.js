...
export async function countRemainingByType(stats = {}) {
  const keys = Object.keys(stats || {}).filter((key) => key !== 'total');
  if (!keys.length) return { total: 0 };

  const result = {};

  await Promise.all(keys.map(async (type) => {
    const total = Number.isFinite(Number(stats[type])) ? Number(stats[type]) : 0;
    if (!total) {
      result[type] = { total: 0, reviewed: 0, remaining: 0 };
      return;
    }
    ...
    const reviewed = Number.isFinite(Number(count)) ? Number(count) : 0;
    const remaining = Math.max(0, total - reviewed);
    result[type] = { total, reviewed, remaining };
  }));

  result.total = Object.values(result).reduce((sum, value) => sum + (value?.remaining || 0), 0);
  return result;
}
